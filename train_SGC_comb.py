...
from models.models import _netDSGCCLS, _netGSGCCLS, _param
...
def train(creative_weight=1000, model_num=1, is_val=True):
    ...
    netGSGCCLS = _netGSGCCLS(dataset.text_dim, dataset.feature_dim).cuda()
    netGSGCCLS.apply(weights_init)
    if model_num == 6:
        netDSGCCLS = _netDSGCCLS(dataset.train_cls_num + 1, dataset.feature_dim).cuda()
    else:
        netDSGCCLS = _netDSGCCLS(dataset.train_cls_num, dataset.feature_dim).cuda()
    netDSGCCLS.apply(weights_init)
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            netGSGCCLS.load_state_dict(checkpoint['state_dict_G'])
            netDSGCCLS.load_state_dict(checkpoint['state_dict_D'])
    ...
    if model_num == 2 or model_num == 4 or model_num == 5:
        nets = [netGSGCCLS, netDSGCCLS, log_SM_ab]
    else:
        nets = [netGSGCCLS, netDSGCCLS]

    tr_cls_centroid = Variable(torch.from_numpy(dataset.tr_cls_centroid.astype('float32'))).cuda()
    optimizerD = optim.Adam(netDSGCCLS.parameters(), lr=opt.lr, betas=(0.5, 0.9))
    optimizerG = optim.Adam(netGSGCCLS.parameters(), lr=opt.lr, betas=(0.5, 0.9))
    if model_num == 2 or model_num == 4 or model_num == 5:
    ...
    for it in tqdm(range(start_step, 3000 + 1)):
        ...
        _, G_creative_sample = netGSGCCLS(z_creative, text_feat_Creative)
        ...
        for _ in range(5):
            ...
            red_text_feature_all, _ = netGSGCCLS(z_all, text_feat_all)  # [160, 1000-d]

            # GAN's D loss
            D_real, C_real, SGC_real = netDSGCCLS(X)
            ...
            SGC_loss_fake = F.cross_entropy(S_c, y_true)
            DC_loss = -D_loss_real + C_loss_real + SGC_loss_fake
            DC_loss.backward(retain_graph=True)

            # GAN's D loss
            _, G_sample = netGSGCCLS(z, text_feat)
            D_fake, C_fake, SGC_fake = netDSGCCLS(G_sample)
            D_loss_fake = torch.mean(D_fake)
            C_loss_fake = F.cross_entropy(C_fake, y_true)
            S_c = torch.matmul(SGC_fake, torch.transpose(red_text_feature_all, 0, 1))
            SGC_loss_fake = F.cross_entropy(S_c, y_true)
            DC_loss = D_loss_fake + C_loss_fake + SGC_loss_fake
            DC_loss.backward(retain_graph=True)

            # train with gradient penalty (WGAN_GP)
            grad_penalty = calc_gradient_penalty(netDSGCCLS, X.data, G_sample.data)
        ...
        for _ in range(1):
            ...
            _, G_sample = netGSGCCLS(z, text_feat)
            D_fake, C_fake, SGC_fake = netDSGCCLS(G_sample)
            _, C_real, SGC_real = netDSGCCLS(X)
            ...
            S_c_false = torch.matmul(SGC_fake, torch.transpose(red_text_feature_all, 0, 1))  # size: [batch_size, 160]
            SGC_loss = (F.cross_entropy(S_c_real, y_true) + F.cross_entropy(S_c_false, y_true)) / 2

            GC_loss = -G_loss + C_loss + SGC_loss
            ...
            if opt.REG_W_LAMBDA != 0:
                for name, p in netGSGCCLS.named_parameters():
                    if 'weight' in name:
            ...
            if opt.REG_Wz_LAMBDA != 0:
                Wz = netGSGCCLS.rdc_text.weight
                reg_Wz_loss = Wz.pow(2).sum(dim=0).sqrt().sum().mul(opt.REG_Wz_LAMBDA)

            # D(C| GX_fake)) + Classify GX_fake as real
            D_creative_fake, C_creative_fake, SGC_creative_fake = netDSGCCLS(G_creative_sample)
            S_c_creative_fake = torch.matmul(SGC_creative_fake, torch.transpose(red_text_feature_all, 0, 1))

            if model_num == 1:  # KL Divergence
                G_fake_C = (F.log_softmax(C_creative_fake) + F.log_softmax(S_c_creative_fake) / 2)
            else:
                G_fake_C = (F.softmax(C_creative_fake) + F.softmax(S_c_creative_fake) / 2)
        ...
        if it % opt.evl_interval == 0 and it > opt.disp_interval:
            netGSGCCLS.eval()
            cur_acc = eval_fakefeat_test(it, netGSGCCLS, dataset, param, result)
            cur_auc = eval_fakefeat_GZSL(netGSGCCLS, dataset, param, out_subdir, result)
            ...
            if cur_auc > result.best_auc:
                ...
                if it % opt.save_interval:
                    ...
                    torch.save({
                        'it': it + 1,
                        'state_dict_G': netGSGCCLS.state_dict(),
                        'state_dict_D': netDSGCCLS.state_dict(),
...
def eval_fakefeat_GZSL(netGSGCCLS, dataset, param, plot_dir, result):
    ...
    for i in range(dataset.train_cls_num):
        ...
        rdc_text_feature, G_sample = netGSGCCLS(z, text_feat)
...
def eval_fakefeat_test(it, netGSGCCLS, dataset, param, result):
    gen_feat = np.zeros([0, param.X_dim])
    for i in range(dataset.test_cls_num):
        text_feat = np.tile(dataset.test_text_feature[i].astype('float32'), (opt.nSample, 1))
        text_feat = Variable(torch.from_numpy(text_feat)).cuda()
...
def calc_gradient_penalty(netDSGCCLS, real_data, fake_data):
    ...
    disc_interpolates, _, _ = netDSGCCLS(interpolates)
