...
parser.add_argument('--cls_weight', type=float, default=1, help='weight of the aux classification loss for the generator')
...
def train(creative_weight=1000, model_num=1, is_val=True):
    ...
    # print(feat_data.shape, feat_data.shape[0])
    # assert 0
    ...
    pretrain_cls = cls.CLS(X, y_true, dataset.train_cls_num, feat_data.shape[1])

    # freezze the cls classifier during the optimization
    ...
    for it in tqdm(range(start_step, 3000 + 1)):
        ...
        for _ in range(1):
            ...
            CLS_loss = cls_criterion(pretrain_cls.model(G_sample), y_true)

            GC_loss = -G_loss + C_loss + opt.cls_weight * CLS_loss
