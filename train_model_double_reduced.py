...
from models.ModelsDoubleReduced import _netDPlus, _netGPlus, _param
...
def train(creative_weight=1000, model_num=1, is_val=True):
    ...
    netG = _netGPlus(dataset.text_dim, dataset.feature_dim).cuda()
    netG.apply(weights_init)
    if model_num == 6:
        netD = _netDPlus(dataset.train_cls_num + 1, dataset.feature_dim).cuda()
    else:
        netD = _netDPlus(dataset.train_cls_num, dataset.feature_dim).cuda()
