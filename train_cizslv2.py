...
from models.models import _netDSGC, _netGSGC, _param
...
def train(creative_weight=1000, model_num=1, is_val=True):
    param = _param() # get z dimension and h dimenstions
    # choose dataset and splitmode
    if opt.dataset == 'CUB':
    ...
    netGSGC = _netGSGC(dataset.text_dim, dataset.feature_dim).cuda()
    netGSGC.apply(weights_init)
    if model_num == 6:
        netDSGC = _netDSGC(dataset.train_cls_num + 1, dataset.feature_dim).cuda()
    else:
        netDSGC = _netDSGC(dataset.train_cls_num, dataset.feature_dim).cuda()
    netDSGC.apply(weights_init)
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            netGSGC.load_state_dict(checkpoint['state_dict_G'])
            netDSGC.load_state_dict(checkpoint['state_dict_D'])
    ...
    if model_num == 2 or model_num == 4 or model_num == 5:
        nets = [netGSGC, netDSGC, log_SM_ab]
    else:
        nets = [netGSGC, netDSGC]

    tr_cls_centroid = Variable(torch.from_numpy(dataset.tr_cls_centroid.astype('float32'))).cuda()
    optimizerD = optim.Adam(netDSGC.parameters(), lr=opt.lr, betas=(0.5, 0.9))
    optimizerG = optim.Adam(netGSGC.parameters(), lr=opt.lr, betas=(0.5, 0.9))
    if model_num == 2 or model_num == 4 or model_num == 5:
    ...
    for it in tqdm(range(start_step, 3000 + 1)):
        ...
        _, G_creative_sample = netGSGC(z_creative, text_feat_Creative)

        """ Discriminator """
        for _ in range(5):
            blobs = data_layer.forward()
            feat_data = blobs['data']  # image data
            labels = blobs['labels'].astype(int)  # class labels, # (batch_size, )
            text_feat = np.array([dataset.train_text_feature[i, :] for i in labels])  # [batch_size, 7551]
            ...
            # get total reduced dim text features
            text_feat_all = np.array(dataset.train_text_feature)
            text_feat_all = Variable(torch.from_numpy(text_feat_all.astype('float32'))).cuda() # [160, 7551]
            z_all = z[:dataset.train_text_feature.shape[0], :] # [160, 100]
            red_text_feature_all, _ = netGSGC(z_all, text_feat_all)  # [160, 1000-d]

            # GAN's D loss for X
            D_real, SGC_real = netDSGC(X) # (batch_size, 1), (batch_size, 1000)
            D_loss_real = torch.mean(D_real)
            # S_c = torch.zeros((opt.batchsize, dataset.train_text_feature.shape[0]))
            # for k in range(opt.batchsize):
            #     # compatible score - inner product
            #     S_c[k, :] = torch.matmul(red_text_feature_all, SGC_real[k, :]).view(1, -1)
            S_c = torch.matmul(SGC_real, torch.transpose(red_text_feature_all, 0, 1)) # size: [batch_size, 160]
            C_loss_real = F.cross_entropy(S_c, y_true)
            DC_loss = -D_loss_real + C_loss_real
            DC_loss.backward(retain_graph=True)

            # GAN's D loss for G_sample
            _, G_sample = netGSGC(z, text_feat)
            D_fake, SGC_fake = netDSGC(G_sample)
            D_loss_fake = torch.mean(D_fake)
            S_c = torch.matmul(SGC_fake, torch.transpose(red_text_feature_all, 0, 1)) # size: [batch_size, 160]
            C_loss_fake = F.cross_entropy(S_c, y_true)
            DC_loss = D_loss_fake + C_loss_fake
            DC_loss.backward(retain_graph=True)

            # train with gradient penalty (WGAN_GP)
            grad_penalty = calc_gradient_penalty(netDSGC, X.data, G_sample.data)
        ...
        for _ in range(1):
            ...
            rdc_text_feature, G_sample = netGSGC(z, text_feat)
            D_fake, SGC_fake = netDSGC(G_sample)
            _, SGC_real = netDSGC(X)
            ...
            S_c_real = torch.matmul(SGC_real, torch.transpose(red_text_feature_all, 0, 1))  # size: [batch_size, 160]
            S_c_fake = torch.matmul(SGC_fake, torch.transpose(red_text_feature_all, 0, 1))  # size: [batch_size, 160]
            # print(S_c_real.shape)
            SGC_loss = (F.cross_entropy(S_c_real, y_true) + F.cross_entropy(S_c_fake, y_true)) / 2

            GC_loss = -G_loss + SGC_loss
            ...
            if opt.REG_W_LAMBDA != 0:
                for name, p in netGSGC.named_parameters():
                    if 'weight' in name:
            ...
            if opt.REG_Wz_LAMBDA != 0:
                Wz = netGSGC.rdc_text.weight
                reg_Wz_loss = Wz.pow(2).sum(dim=0).sqrt().sum().mul(opt.REG_Wz_LAMBDA)

            # D(C| GX_fake)) + Classify GX_fake as real
            # print(G_creative_sample.shape)
            # assert 0
            D_creative_fake, SGC_creative_fake = netDSGC(G_creative_sample)
            S_c_creative_fake = torch.matmul(SGC_creative_fake, torch.transpose(red_text_feature_all, 0, 1))

            if model_num == 1:  # KL Divergence
                G_fake_C = F.log_softmax(S_c_creative_fake)
            else:
                G_fake_C = F.softmax(S_c_creative_fake)
            ...
            if model_num == 6:
                loss_creative = F.cross_entropy(S_c_creative_fake, new_class_labels)
            else:
                if model_num != 1:
        ...
        if it % opt.disp_interval == 0 and it:
            acc_real = (np.argmax(S_c_real.data.cpu().numpy(), axis=1) == y_true.data.cpu().numpy()).sum() / float(
                y_true.data.size()[0])
            acc_fake = (np.argmax(S_c_fake.data.cpu().numpy(), axis=1) == y_true.data.cpu().numpy()).sum() / float(
                y_true.data.size()[0])
        ...
        if it % opt.evl_interval == 0 and it > opt.disp_interval:
            netGSGC.eval()
            cur_acc = eval_fakefeat_test(it, netGSGC, dataset, param, result)
            cur_auc = eval_fakefeat_GZSL(netGSGC, dataset, param, out_subdir, result)
            ...
            if cur_auc > result.best_auc:
                ...
                if it % opt.save_interval:
                    ...
                    torch.save({
                        'it': it + 1,
                        'state_dict_G': netGSGC.state_dict(),
                        'state_dict_D': netDSGC.state_dict(),
...
def eval_fakefeat_GZSL(netGSGC, dataset, param, plot_dir, result):
...
def eval_fakefeat_test(it, netGSGC, dataset, param, result):
    gen_feat = np.zeros([0, param.X_dim])
    for i in range(dataset.test_cls_num):
        text_feat = np.tile(dataset.test_text_feature[i].astype('float32'), (opt.nSample, 1))
        text_feat = Variable(torch.from_numpy(text_feat)).cuda()
...
def calc_gradient_penalty(netDSGC, real_data, fake_data):
    ...
    disc_interpolates, _ = netDSGC(interpolates)
