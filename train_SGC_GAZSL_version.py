...
import models.classifier2 as classifier2
import models.sgc as sgc
...
parser.add_argument('--sgc_weight', type=float, default=1, help='weight of the semantic guided matrix loss')
...
def train(creative_weight=1000, model_num=1, is_val=True):
    ...
    # semantic guided categorizer loss
    sgc_criterion = nn.LogSoftmax()
    ...
    # train a semantic guided matrix on seen classes
    # 1: get data and labels
    blobs = data_layer.forward()
    feat_data = blobs['data']  # image data: [1000, 3584]
    labels = blobs['labels'].astype(int) # corresponding labels: [1000, ]
    ...
    # print(text_feat.shape, X.shape, y_true.shape) # torch.Size([1000, 7551]) torch.Size([1000, 3584]) torch.Size([1000])

    # _train_X, _train_Y, _nclass, _input_dim,
    # pretrain_cls = sgc.SGC(X, y_true, text_feat, dataset.train_cls_num, feat_data.shape[1])
    pretrain_cls = sgc.SGC(X, y_true, text_feat, dataset.train_cls_num, feat_data.shape[1])

    # freezze the sgc classifier during the optimization
    for p in pretrain_cls.model.parameters(): # set requires_grad to Flase
    ...
    for it in tqdm(range(start_step, 3000 + 1)):
        ...
        for _ in range(1):
            ...
            # semantic guided categorizer, apply to generated features only
            # pretrain_cls = sgc.CLASSIFIER(X, y_true, dataset.train_cls_num, feat_data.shape[1])
            # SGC_loss = sgc_criterion(pretrain_cls.model(G_sample), y_true)
            generated_text_embedding = pretrain_cls.model(G_sample)
            bmm1 = generated_text_embedding.view(generated_text_embedding.shape[0], 1, generated_text_embedding.shape[1])
            bmm2 = text_feat.view(text_feat.shape[0], text_feat.shape[1], 1)
            Sc = torch.bmm(bmm1, bmm2)
            SGC_loss = sgc_criterion(Sc)
            SGC_loss = torch.mean(SGC_loss)

            GC_loss = -G_loss + C_loss + opt.sgc_weight * SGC_loss
